...
function createQualitySelector(qualityOptions, player) {
  const selector = document.createElement('select')
  qualityOptions.forEach((option) => {
    const opt = document.createElement('option')
    opt.value = option.url.value
    opt.innerText = option.label
    opt.setAttribute('data-type', option.url.type)
    selector.appendChild(opt)
  })

  selector.addEventListener('change', async function () {
    const selected = this.value
    const selectedType = this.selectedOptions[0]?.getAttribute('data-type')
    const matched = qualityOptions.find((opt) => opt.url.type === selectedType)

    if (matched && matched.url.type === 'Local') {
      player.src({ type: 'video/mp4', src: matched.url.value })
      player.load()
      await player.play()
    } else {
      const response = await fetch(`${baseUrl}/video/stream/${encodeURIComponent(selected)}`)
      const data = await response.json()
      setVideoSource(player, data.platform, data.videoId, data.url, data.mimeType, qualityOptions)
      player.load()
      await player.play()
